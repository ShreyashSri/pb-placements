...
export async function analyzeWithMistral(text: string, extractedLinks: string[] = []): Promise<ParsedResumeData> {
  const prompt = `
    ...
    6. A list of notable achievements. ONLY extract from a section explicitly labeled "Achievements" , "Awards", "Honors", or "Accomplishments" in the resume. If no such section exists or no achievements are explicitly listed under it, return an empty array []. Do NOT extract from projects, experiences, or certifications sections. Do NOT infer, guess, or hallucinate achievements. [Dont put any dates for achievements]
